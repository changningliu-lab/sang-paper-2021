...
#参数2：plant(animal)_cluster_genename(输出物种_cluster_genename/chr_strand_start_end_genename.txt;物种_cluster_genename.txt；物种_cluster_single_genename.txt文件的路径),这两个文件只是为了留下中间过程数据
    ...
    for (genename2,location2) in value[1:]:        
        ...
        if start2 > end_index:
            ...
            for i in range(len(cpvalue)-1,-1,-1):
                ...
                if overlap/min(length1,length2) >= 0.5:
